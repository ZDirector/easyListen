...
class MusicResultFragment : BaseFragment<FragmentMusicResultBinding, MusicResultViewModel>() {
    ...
    override fun initData(savedInstanceState: Bundle?) {
        ...
        playViewModel = MyApplication.getAppViewModel(PlayViewModel::class.java)
        initView()
        initListener()
        initObserve()
    }
            ...
            if (it.isNullOrEmpty()) {
                ...
                if (binding.musicResultList.adapter == null) {
                    ...
                    adapter.setItemOnClickListener(object :
                        MusicResultListAdapter.ItemOnClickListener {
                        override fun onClick(view: View, i: Int) {
                            when (view.id) {
                                R.id.item_search_music_setting -> {//当点击音乐列表item中的设置图标时显示弹窗
                                    val popupWindow =
                                        MusicPopupWindow(view, adapter.musicResultList[i])
                                    ...
                                    playViewModel.getMusicUrl(adapter.musicResultList[i].id) { list ->
                                        list.forEach { musicUrl ->
                                            if (musicUrl.id == adapter.musicResultList[i].id) {
                                                val music =
                                                    adapter.musicResultList[i].trans2MusicBean(
                                                        musicUrl.url
                                                    )
