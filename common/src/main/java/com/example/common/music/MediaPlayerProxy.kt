...
import com.example.common.utils.IoUtils
import java.io.*
import java.net.*
 ...
 * 本地播放器代理，用于边下边播。
 */
class MediaPlayerProxy private constructor(
    private var audioCachePath: String, //是否缓存播放文件
    private var needCacheAudio: Boolean = true
) : MediaPlayer() {
        ...
        return try {
            ...
            if (remoteHost.isNotEmpty()) {
                if (originalURI.port != -1) { //URL带Port
                    ...
                    localProxyUrl = url.replace(
                        remoteHost + ":" + originalURI.port,
        ...
        try { //获取音乐网络数据
            ...
            try {
                ...
                while (in_remoteSocket.read(remote_reply, 0, remote_reply.size).also
                    { readLenth = it } != -1 && currProxyId == latestProxyId
                ) { //首先从数据中获得文件总长度
                    try {
                        if (firstData) {
                            firstData = false
                    ...
                    try {
                        ...
                        if (fileTotalLength > 0 && currProxyId == latestProxyId) {
                            currMusicCachedProgress =
                                (cachedFileLength * 100.0f / fileTotalLength).toInt()
                            if (onCachedProgressUpdateListener != null && currMusicCachedProgress <= 100) {
                                onCachedProgressUpdateListener!!.updateCachedProgress(
                                    currMusicCachedProgress
                                )
                            }
                        }
                ...
                if (fileOutputStream != null) {
                    ...
                    if (theFile != null && theFile.exists()) {
                        ...
                        if (musicControlInterface != null) {
                            musicControlInterface!!.updateBufferFinishMusicPath(
                                musicKey,
                                theFile.path
                            )
                            bufferingMusicUrlList.remove(remoteUrl)
    ...
    init {
        try {
            if (localServerSocket == null || localServerSocket!!.isClosed) {
                ...
                val socketAddress =
                    InetSocketAddress(InetAddress.getByName(LOCALHOST), localProxyPort)
            ...
            try {
                localProxyPort++
                localServerSocket =
                    ServerSocket(localProxyPort, 0, InetAddress.getByName(LOCALHOST))
                localServerSocket!!.reuseAddress = true
            } catch (e2: Exception) {
                e2.printStackTrace()
            }
